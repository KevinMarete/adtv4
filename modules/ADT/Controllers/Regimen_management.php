...
class Regimen_management extends BaseController {
	...
	public function save() {
		...
		// check for duplicate code before 
			...
			$this->session->set('msg_error', 'Regimen Code already exists!');
			...
			redirect('settings_management');
				...
				$this->session->set('msg_success', $this -> input -> post('regimen_code') . ' was added.');
	...
	public function enable($regimen_id) {
		if($this ->input ->post('multiple')){
			...
			if($this -> db -> query($the_query)){
				$this->session->set('msg_success','The selected regimens were successfully enabled!');
			}else{
				$this->session->set('msg_error', 'One or more of the selected regimens were not enabled!');
			}
		}else{
			$query = $this -> db -> query("UPDATE regimen SET enabled='1'WHERE id='$regimen_id'");
			$results = Regimen::getRegimen($regimen_id);
			$this->session->set('message_counter', '1');
			$this->session->set('msg_success', $results -> Regimen_Code . ' was enabled');
			$this -> session -> set_flashdata('filter_datatable', $results -> Regimen_Code);
			//Filter
	...
	public function disable($regimen_id) {
		if($this ->input ->post('multiple')){
			...
			$this->session->set('msg_error', $results -> Regimen_Code . ' was disabled');
			$this -> session -> set_flashdata('filter_datatable', $results -> Regimen_Code);
			//Filter
			redirect('settings_management');
		}
	...
	public function merge($primary_drugcode_id) {
		...
		$this->session->set('message_counter', '1');
		$this->session->set('msg_success', $results -> Regimen_Code . ' was Merged');
	}
	...
	public function unmerge($drugcode) {
		...
		$this->session->set('message_counter', '1');
		$this->session->set('msg_error', $results -> Regimen_Code . ' was Unmerged');
		redirect('settings_management');
	...
	public function regimens(){
		$regimens = Regimen::getAllHydrated();
