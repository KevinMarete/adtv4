...
class Migration_management extends BaseController {
	...
	public function index() {
		...
		$data['stores'] =(array) DB::select($sql);
		//get databases in server
		$sql = "SHOW DATABASES";
		$data['databases'] = (array) DB::select($sql);
		//get tables
		$data['tables'] = $this->mapping();
		//migration view
		$data['content_view'] = "\Modules\ADT\Views\\migration_v";
 ...
 */
	public function getFacilities(){
        ...
        $sql = "SELECT facilitycode as facility_code,name as facility_name  FROM facilities ".
                "WHERE name IS NOT NULL  AND name !='' AND name LIKE '%".$q."%' ".
                "ORDER BY name ASC";
		$results = (array) DB::select($sql);

		if($results){
           foreach($results as $result){
           	 $answer[] = ["id"=>$result['facility_code'],"text"=>$result['facility_name']];
           }
		}else{
	...
	public function migrate(){
		//get posted data
		$facility_code=$this->post('facility_code',TRUE);
		$ccc_pharmacy=$this->post('ccc_pharmacy',TRUE);
		$source_database=$this->post('source_database',TRUE);
		...
		if($migration_id !=null){
			...
			Migration_log::where('id', $migration_id)->update($migration_log);
		}else{
			$migration_log = [
            ...
            Migration_log::create('migration_log',$migration_log);
		}
