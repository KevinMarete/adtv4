...
class Order_settings extends BaseController {
	...
	public function listing($table = "") {
		...
		//Load table data
		$results = DB::select($params[$table]['query']);
		...
		foreach($results as $result){
			...
			foreach($result as $index => $value){
				if($index == 'Active'){
					$edit_link = anchor('#'.$table.'_form', 'Edit', ['id' => $result->id, 'table' => $table, 'role' => 'button', 'class' => 'edit_setting', 'data-toggle' => 'modal']);
					$disable_link = anchor('order_settings/disable/' . $table . '/' . $result->id, 'Disable', ['class' => 'disable_user']);
					$enable_link = anchor('order_settings/enable/' . $table . '/' . $result['id'], 'Enable', ['class' => 'enable_user']);;
					$links = $edit_link;
	...
	public function save($table = ''){
		...
		if($this->db->affected_rows() > 0){
			$this->session->set('msg_success', $this -> input -> post('name') . ' was successfully Added!');
		}else{
			$this->session->set('msg_error', $this -> input -> post('name') . ' was not Added!');
	...
	public function update($table = '', $id = ''){
		...
		if($this->db->affected_rows() > 0){
			$this->session->set('msg_success', $this -> input -> post('name') . ' was successfully Updated!');
		}else{
			$this->session->set('msg_error', $this -> input -> post('name') . ' was not Updated!');
