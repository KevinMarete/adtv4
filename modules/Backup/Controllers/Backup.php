...
//$ftp = new \Ftp();
        ...
        $backup_limit = 5;
        $data['active_menu'] = 2;
        $data['content_view'] = "\Modules\Backup\Views\\backup_v";
...
// table html string
        ...
        if (!is_array($data['remote_files'])) {
            $data['ftp_status'] = "$('.alert').addClass('alert-danger');$('.alert').text('Cannot connect to remote server');$('.alert').show();$('.upload').attr('disabled',true);";
        }
...
//die;
        ...
        $db = \Config\Database::connect();
        $session = session();
        $hostname_port = explode(':', $db->hostname)[0];
...
//$CI->load->database();
        ...
        $list = $ftp->list_files($this->ftp_root . '/');


        if (!in_array($this->ftp_root . '/' . $facility_code . '', $list)) {
            $ftp->mkdir($this->ftp_root . $facility_code . '/', 0755);
        }
...
// $destination = $file;
    ...
    public function zip_backup($filename = null, $file_path = null) {
        ...
        $zip->read_file($filename, FALSE);
// Write the zip file to a folder on your server. Name it "my_backup.zip"
        if ($zip->archive($file_path . '.zip')) {
            return true;
        }
    }
    ...
    public function connect_ftp() {
        $ftp = new \Ftp();
        if ($ftp->connect($this->ftp_config)) {
            return true;
        } else {
            return false;
        }
