$(document).ready(function () {
    ...
    $('.phone').change(function () {
         ...
         * 2.If yes,check if first characters are 07 or 01{
         * 3.if matches 07 or 01 alert successful
         * 4.if no match alert your phone number should start with 07 or 01}
         * 5.if no,alert incorrect phone format used
         */
        if (phone_length == number_length) {
            var first_char = phone.substr(0, 2);
            if (first_char != 07 && first_char != 01) {
                alert('your phone number should start with 07 or 01');
            }
        } else {
    ...
    $('.dataTable tbody td').live('mouseover', function () {
        //Show full text when one mouseovers
        var sTitle;
        sTitle = jQuery(this).text();
        this.setAttribute('title', sTitle);
    });
    $('.listing_table tbody td').live('mouseover', function () {
        //Show full text when one mouseovers
        var sTitle;
        sTitle = jQuery(this).text();
        this.setAttribute('title', sTitle);
...
function getPeriodDrugBalance(count, start_date, facility_id, code, total, drugs, stores) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#positive_adjustment_' + drug).attr('value', data.adjustments);
            $('#adjustments_in_period_' + drug).attr('value', data.adjustments_neg);
...
function getPeriodRegimenPatients(start_date, end_date) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var total_patients_male_div = '';
            var total_patients_female_div = '';
            $.each(data, function (i, jsondata) {
                ...
                total_patients_male_div = '#patient_numbers_male' + jsondata.regimen;
                $(total_patients_male_div).attr('value', jsondata.male);
                total_patients_female_div = '#patient_numbers_female' + jsondata.regimen;
                $(total_patients_female_div).attr('value', jsondata.female);
                //Calculate total summary of ART patients

                if (regimen_category.indexOf('pep') > -1 || regimen_category.indexOf('pmtct') > -1) {
                } else if (
                        regimen_category.indexOf('paed') > -1 ||
                        regimen_category.indexOf('ped') > -1 ||
                        regimen_category.indexOf('child') > -1
                        ) {
                    //Check if regimen is adult or paed
                    var old_val = $('#art_child').val();
                    var new_val = parseInt(old_val) + parseInt(total_patients);
                    $('#art_child').val(new_val);
                } else if (regimen_category.indexOf('adult') > -1 || regimen_category.indexOf('mother') > -1) {
                    //Adult regimen
                ...
                $('#tbl_patients_regimen')
                        ...
                        .each(function () {
                            ...
                            if (regimen_id == regimen_desc) {
                                ...
                                $(this).find('.regimen_females').find('.patient_number_female').attr('value', jsondata.female);
                                $(this).find('.regimen_males').find('.patient_number_male').attr('value', jsondata.male);
                            }
                        });
            });
        },
...
$(document).ready(function () {
    ...
    $('.generate_btn').live('click', function () {
        var base_url = $('#base_url').val() + '/public/';
        if (
                $('.input-medium').is(':visible') ||
                $('.month_period').is(':visible') ||
                $('.report_type').is(':visible') ||
                $('.report_type_1').is(':visible') ||
                $('.input_year').is(':visible') ||
                $('.input_dates').is(':visible') ||
                $('.donor_input_dates_from').is(':visible') ||
                $('.input_dates_from').is(':visible') ||
                $('.donor_input_dates_to').is(':visible') ||
                $('.input_dates_to').is(':visible')
                ) {
...
//Dropdown not chosen
                ...
                alert('Please select the report type');
                    ...
                    $('#commodity_summary_report_type_1').is(':visible') &&
                    $('#commodity_summary_report_type_1').val() == 0
                    ) {
                alert('Please select the report type');
            }
//If everything is ok,generate a report
                ...
                if (id == 'generate_date_range_report') {
                    ...
                    if ($('.select_report').val() == 'clinical_bands') {
                        report_url =
                                base_url + 'report_management/' + report + '/' + from + '/' + to + '/' + gender + '/' + clinicalage;
                    }
...
//Visiting report tab selected
            ...
            $(this).addClass('active');
            $('.reports_types').css('display', 'none');
            var report_url = $('#base_url').val() + '/public/report_management/load_guidelines_view';
            window.location = report_url;
        }
    });
    //Features to select
    $('.select_report').change(function () {
        ...
        if (get_type == 'donor_date_range_report') {
            ...
            if (selected == 'graphical_adherence') {
//If adherence report is selected, show  select adherence report type
...
//If report is drug_consumption report, display select report type
            if (
                    get_id == 'drug_stock_on_hand' ||
                    get_id == 'expiring_drugs' ||
                    get_id == 'expired_drugs' ||
                    get_id == 'getDrugsIssued' ||
                    get_id == 'getDrugsReceived' ||
                    get_id == 'commodity_summary'
                    ) {
