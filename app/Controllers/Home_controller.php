...
class Home_controller extends \CodeIgniter\Controller {
    ...
    public function home() {
        ...
        $this->session->set_userdata(['download_status' => $data['download_status']]);
        $data['update_available'] = json_decode($this->updater->check_ADTrelease());
        if (str_replace('.', '', $data['update_available']->release) + 0 > str_replace('.', '', $this->config->item('adt_version')) + 0) {
            $this->session->set_userdata(['update_available' => true]);
        }
