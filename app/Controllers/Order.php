...
class Order extends MY_Controller {
			...
			if(!empty($missing_dhiscode)){
				...
				if (!$curl -> error) {
					...
					foreach ($org_response['organisationUnits'] as $org) {
						if(isset($org['code'])){
							$this->db->where('code', $org['code']);
							$this->db->update('sync_facility', array('dhiscode' => $org['id']));
						}
					}
				}
			}
	...
	public function get_orders($type = "cdrr", $period_begin = "") {
		$columns = array('#', '#ID', 'Period Beginning', 'Status-Online', 'Facility Name', 'Options');
	...
	public function create_order($type = "cdrr", $order_type, $content_array = []) {
		...
		if ($type == "cdrr") {
			...
			if (!empty($content_array)) {
				...
				if ($data['options'] == "update") {
					...
					$sql_regimen = "SELECT rc.id,r.id as reg_id,rc.Name as name,$regimen_code,$regimen_desc,$regimen_cat_join,mi.total, mi.male, mi.female
	...
	public function save($type = "cdrr", $status = "prepared", $id = "") {
		...
		if ($type == "cdrr") {
			...
			if ($save) {
				...
				if ($id != "") {
					...
					if ($this -> input -> post("status_change")) {
						$status = $this -> input -> post("status_change");
						$cdrr_type = $this->input->post("cdrr_type");
						if($status =='approved'){ $this->upload_dhis($cdrr_type ,$id); }
					}
		...
		if ($type == "maps") {
			...
			if ($save) {
				...
				$patient_numbers_male = $this->input->post('patient_numbers_male');
				$patient_numbers_female = $this->input->post('patient_numbers_female');
				//insert map
				$main_array['id'] = $id;
				...
				if ($id != "") {
					...
					if ($this -> input -> post("status_change")) {
						...
						if($status =='approved'){ $this->upload_dhis($maps_type ,$id); }
					}
	...
	public function getoiPatients() {
		...
		//Begin fluconazole
		$sql = "SELECT 
			ROUND(DATEDIFF(CURRENT_DATE, p.dob)/365) AS age, 
		...
		$oi_patients[] = array('OI1AM'=>$a,'OI1AF'=>$b,'OI1A'=>$c,'OI1CM'=>$d,'OI1CF'=>$e,'OI1C'=>$f,'OI2AM'=>$g,'OI2AF'=>$h,'OI2A'=>$i,'OI2CM'=>$j,'OI2CF'=>$k,'OI2C'=>$l,'ATPT1AM'=>$m,'ATPT1AF'=>$n,'ATPT1A'=>$o,'CTPT1AM'=>$p,'CTPT1AF'=>$q,'CTPT1A'=>$r,'OI5AM'=>$sa,'OI5AF'=>$sb,'OI5A'=>$sc,'OI5CM'=>$sd,'OI5CF'=>$se,'OI5C'=>$sf,'ATPT1BM'=>$u,'ATPT1BF'=>$v,'ATPT1B'=>$w,'CTPT1BM'=>$x,'CTPT1BF'=>$y,'CTPT1B'=>$z,'OI6AM'=>$za,'OI6AF'=>$zb,'OI6A'=>$zc);
	...
	public function getItems() {
        ...
        $exempted_columns = array('expiry_month','beginning_balance','pack_size','reported_consumed','reported_physical_stock');
		foreach ($row as $i => $v) {
	...
	public function upload_dhis($order_type,$order_id){
		...
		if ($order_type == 'maps' || $order_type == 'fmaps' || $order_type == 'dmaps') {
			...
			$dhismessage = array(
				'dataSet'=> $order_type == 'dmaps' ? $this->config->config['dhiscode']['dmaps_code'] : $this->config->config['dhiscode']['fmaps_code'],
			...
			$query = $this->db->get_where('sync_facility', ['id' =>  $results['facility_id']]);
			...
			foreach ($results['item'] as $key => $item) {
				...
				if ($item['balance']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['balance'], 'value' =>$item['balance']];
				}
				if ($item['received']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['received'], 'value' =>$item['received']];
				}
				if ($item['dispensed_packs']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['dispensed_packs'], 'value' =>$item['dispensed_packs']];
				}
				if ($item['losses']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['losses'], 'value' =>$item['losses']];
				}
				if ($item['adjustments']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['adjustments'], 'value' =>$item['adjustments']];
				}
				if ($item['adjustments_neg']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['adjustments_neg'], 'value' =>$item['adjustments_neg']];
				}
				if ($item['count']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['count'], 'value' =>$item['count']];
				}
				if ($item['expiry_quant']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['expiry_quant'], 'value' =>$item['expiry_quant']];
				}
				if ($item['expiry_date']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['expiry_date'], 'value' =>$item['expiry_date']];
				}
				if ($item['out_of_stock']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['out_of_stock'], 'value' =>$item['out_of_stock']];
				}
				if ($item['resupply']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['resupply'], 'value' =>$item['resupply']];
				}
				if ($item['count']!==null){
					$dataValues[] = ['dataElement' =>$item['dhis_code'],'categoryOptionCombo' =>$this->config->config['dhiscode']['total'], 'value' =>$item['count']];
				}

			}
			$dhismessage = [
				'dataSet'=> $order_type == 'dcdrr' ? $this->config->config['dhiscode']['dcdrr_code'] : $this->config->config['dhiscode']['fcdrr_code'],
				'completeDate' => date('Y-m-d',strtotime($results['updated'])),
				'period' => date('Ym',strtotime($results['period_begin'])),
				'orgUnit'=> $dhis_org,
				'attributeOptionCombo'=> "NhSoXUMPK2K",
	...
	public function get_sync_facility_id($dhis_code, $code){
		...
		if(in_array($code, array('F-CDRR_units', 'F-MAPS'))  && $this->facility_type != 1){ //Satellite
			$result = $this->db->select('id')->get_where('sync_facility', array(
				'dhiscode' =>  $dhis_code, 
				'category' => 'satellite', 
				'ordering' => '0',
				'service_point' => '1'
			))->row_array();
		}else if(in_array($code, array('F-CDRR_packs', 'F-MAPS'))  &&  $this->facility_type == 1){ //Standalone
			$result = $this->db->select('id')->get_where('sync_facility', array(
				'dhiscode' =>  $dhis_code, 
				'category' => 'standalone', 
				'ordering' => '1',
		...
		}else if(in_array($code, array('D-CDRR', 'D-MAPS')) &&  $this->facility_type > 1){ //Central
			$result = $this->db->select('id')->get_where('sync_facility', array(
				'dhiscode' =>  $dhis_code, 
				'category' => 'central', 
				'ordering' => '1',
