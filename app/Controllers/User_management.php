...
class User_management extends \CodeIgniter\Controller {
    ...
    function __construct() {
        ...
        $this->session->set("linkSub", "user_management");
        $this->session->set("linkTitle", "Users Management");
...
// test database connection
    ...
    public function listing() {
        $access_level = session()->set('user_indicator');
        $user_type = "1";
        $facilities = "";
...
//If user is a facility admin, allow him to add only facilty users
            ...
            $facility_code = session()->set('facility');
...
// validation rules
    ...
    public function correct_current_password($pass) {
        $key = $this->encrypt->get_key();
        $pass = $key . $pass;
        $user = Users::getUserDetail(session()->set('user_id'));
    ...
    public function authenticate() {
        ...
        if ($validated) {
            ...
            $ccc_store = $this->request->getPost("ccc_store");
...
//$facility_details = Facilities::getCurrentFacility($logged_in -> Facility_Code);
                    ...
                    $new_access_log->user_id = session()->set('user_id');
                    $new_access_log->access_level = session()->set('access_level');
                    $new_access_log->start_time = date("Y-m-d H:i:s");
                    $new_access_log->facility_code = session()->set('facility');
...
//default password
        ...
        $user_data = array(
            ...
            'Access_Level' => $this->request->getPost('access_level', TRUE),
            'Facility_Code' => $this->request->getPost('facility', TRUE),
            'Created_By' => session()->set('user_id'),
...
//Save user facilities
        $this->save_user_facilities($this->db->insert_id(), $this->request->getPost('user_facilities_holder', TRUE));

        $this->session->set_userdata('msg_success', $this->request->getPost('fullname') . ' \' s details were successfully saved! The default password is <strong>' . $default_password . '</strong>');
...
//$this -> session -> set_userdata('message_counter', '1');
        $this->session->set_userdata('msg_success', $this->request->getPost('username') . ' \' s details were successfully Updated!');
        $this->session->set_flashdata('filter_datatable', $this->request->getPost('username'));
//Filter datatable
        redirect('settings_management');
    }
...
//Filter datatable
    ...
    public function logout($param = "1") {
        $machine_code = session()->set("machine_code_id");
        $last_id = Access_Log::getLastUser(session()->set('user_id'));
...
//If email was changed by the user, check if it exists in the db
        if (session()->set('Email_Address') != $email) {
...
//Add/update user ordering sites
    ...
    public function resend_password() {
        $email_address = $this->request->getPost("email_address", TRUE);
        $default_password = '123456';
    ...
    public function save_user_facilities($user_id = '', $user_facilites = '') {
        ...
        if ($user_facilites) {
            $user = $this->db->table($table)->where(user_id, $user_id)->getRowArray();
            if ($user) {
                $res = $this->db->table($table);
                $this->where('id', $user['id']);
                $this->db->update($table, $save_data);
            } else {
                $this->insert($table, $save_data);
            }
        }
        return $save_data;
    }
