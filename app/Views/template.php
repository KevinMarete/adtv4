 ...
 */
$session = session();
if (!$session->get('user_id') && $content_view != 'resend_password_v') {
    redirect()->to("login");
}
if (!isset($link)) {
    $link = null;
...
$actual_page = $uri->getSegment(1);
if ($uri->getSegment(2) != "") {
    $actual_page .= "/" . $uri->getSegment(2);
}
if ($uri->getSegment(3) != "") {
    $actual_page .= "/" . $uri->getSegment(3);
}
if ($uri->getSegment(4) != "") {
    $actual_page .= "/" . $uri->getSegment(4);
}
if ($uri->getSegment(5) != "") {
    $actual_page .= "/" . $uri->getSegment(5);
}
if ($uri->getSegment(6) != "") {
    $actual_page .= "/" . $uri->getSegment(6);
}
if ($uri->getSegment(7) != "") {
    $actual_page .= "/" . $uri->getSegment(7);
}
            ...
            <div class="navbar-inner" style="background:white">
                <div class="container-fluid" style="padding-bottom:1.2em;">
                    ...
                    <div class="welcome_msg">
                        <?php if ($this->session->userdata('update_available')) { ?>
                        ...
                        <?php } ?>
                        <span><span style="color:#B8B8B8; text-transform: uppercase; font-weight: bold;" >username: </span><b style="font-weight: bold;font-size: 15px;text-transform: uppercase;"><?php echo $this->session->userdata('full_name'); ?> | </b><a id="logout_btn" href="<?php echo base_url() . 'user_management/logout/2' ?>"><i class="icon-off"></i> Logout</a></span>
                        <br>
